...
<ParamField path="DEEPSEEK_SEARCH_API_KEY" type="string">
  DeepSeek API key used for native web search. Enables real-time web search during scans for OSINT and vulnerability research. Falls back to `DEEPSEEK_API_KEY` when unset, so a single key can cover both inference and search.
</ParamField>

<ParamField path="DEEPSEEK_SEARCH_API_BASE" default="https://api.deepseek.com/anthropic/v1" type="string">
  Endpoint base for web search; `/messages` is appended. Point this at a trusted Anthropic-compatible Messages API to route search through a different gateway than chat.
</ParamField>

<ParamField path="DEEPSEEK_SEARCH_MODEL" default="deepseek-v4-flash" type="string">
  Model that performs the search and summarizes the retrieved pages.
</ParamField>
...
export DEEPSEEK_SEARCH_API_KEY="sk-..."  # falls back to DEEPSEEK_API_KEY
