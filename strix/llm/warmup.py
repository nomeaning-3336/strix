...
def wait_for_import_warmup() -> None:
    ...
    The main thread calls this before its first import from the scan-engine
    graph. Without it, the warm-up thread (importing ``strix.core.runner``)
    and the main thread (importing e.g. ``agents.models.interface``) can enter
    the same packages from different entry points, which CPython resolves by
    failing one of the imports.
    """
    thread = _thread
