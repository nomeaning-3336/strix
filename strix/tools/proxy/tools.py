...
async def existing_request_ids(
    ...
    Used to validate ``http_exchange_ids`` on a finding before it is stored:
    a stale or mistyped id would otherwise become evidence that can never be
    opened. Raises when the proxy is unavailable, so the caller can decide
    whether to reject the ids or drop them with a warning.
    """
    if not request_ids:
        return set()
