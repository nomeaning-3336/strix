...
Two threads that enter the same package graph from different modules hold each
other's import locks (warm-up: ``strix.core.runner`` -> ``agents``; main:
``agents.models.interface``). CPython breaks such a cycle by failing one of the
imports, so the main thread waits for the warm-up before its first engine
import.

``strix.report`` must stay importable without dragging the agents graph in —
that was the fork's original boot-cycle failure, fixed by lazy report imports.
"""
...
def test_failed_warm_import_does_not_raise() -> None:
    """A module that cannot import must not fail the run (no purge needed)."""
    warmup._warm(("strix_no_such_module_for_warmup_test",))


def test_warmup_does_not_include_docker() -> None:
    """Docker is imported by the main-thread checks during the same window."""
    assert "docker" not in warmup.WARMUP_MODULES


def test_wait_for_import_warmup_is_a_no_op_without_a_thread(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(warmup, "_thread", None)
